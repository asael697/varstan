...
Varstan is a package for analyze time series with common structured models such as arima, garch, var, bekk, SVM, HMM, ETS and others. On this beta version the models varma, garch, Bekk, Sarima are currently available.

The dynamic is, first to create the model structure using a constructor, then define the prior parameters, next call the varstan function to estimate the chosen model in a Bayesian scheme using STAN. Finally the summary, point_estimate, model_diagnostic, forecast functions are used to evaluate the obtained results.

The posterior distribution is approximated using a Hamiltonian Monte Carlo, using the stan using the r package rstan.
...
For avoiding package installing problems, make shure you have updated the next packages: **devtools,pkgbuild,remotes,StanHeaders,OpenMx**.If you are not sure about it, compile the code bellow 
...
install_github("asael697/varstan", upgrade='never',INSTALL_opts = "--no-multiarch", 
